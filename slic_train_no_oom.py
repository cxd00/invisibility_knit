...
class SLICGenerator(torch.nn.Module):
    ...
    def differentiable_slic(self, image, n_segments=200, compactness=10.0, temperature=0.1, requires_grad=True):
        ...
        # Initialize centroids by sampling random pixels
        N = features.shape[0]
        indices = torch.randperm(N)[:n_segments]
        ...
        for _ in range(3):  # Reduced from 5
            ...
            weights_sum = assignments.sum(dim=0).unsqueeze(1)
            
            # Process centroid updates in chunks
...
def print_memory_usage(context: str):
    ...
    if torch.cuda.is_available():
        print(f"GPU Memory allocated at {context}: {torch.cuda.memory_allocated()/1e9:.2f} GB")
        print(f"GPU Memory cached at {context}: {torch.cuda.memory_reserved()/1e9:.2f} GB")
    
    # CPU Memory
    process = psutil.Process()
    print(f"CPU Memory used at {context}: {process.memory_info().rss/1e9:.2f} GB")
...
if __name__ == "__main__":
    ...
    # Initialize trainer
    trainer = PatchTrainer(batch_size=4, img_size=416)
    # Train SLIC
    image_path = "test.png"
